...
7. **Eval Dataset Creation:** Created evaluation datasets for the InterPLM pipeline ensuring reproducibility with the following parameters:
   ...
   * **67k Dataset:**
     ...
     * Result: Filtered from 250 concepts to 219 concepts with at least 1,500 amino acids or 25 domains.
...
8. **Embedding Generation:** Generated ProtT5 embeddings for the swissprot sequences.
   ...
   * **67k Dataset:** Ran `InterPLM/scripts/submit_embed.sh` containing `InterPLM/scripts/embed_annotations.py` on an `lrz-hgx-h100-94x4` node to generate ProtT5 embeddings for the 67k swissprot sequences in `/dss/dssfs02/lwp-dss-0001/pn67na/pn67na-dss-0000/ga25ley2/data/uniprotkb_modern_score45_67k/`.
...
9. **Crosscoder Feature Normalization:** Normalized the crosscoder feature activation values between 0 and 1.
   ...
   * **67k Dataset:** Ran `InterPLM/scripts/submit_normalize.sh` containing `InterPLM/interplm/sae/normalize.py` on an `lrz-hgx-h100-94x4` node to normalize using the 67k swissprot sequences in `/dss/dssfs02/lwp-dss-0001/pn67na/pn67na-dss-0000/ga25ley2/data/uniprotkb_modern_score45_67k/`.
   * **Baseline (67k):** Ran `InterPLM/scripts/submit_normalize.sh` for the baseline crosscoder (trained on randomized ProtT5) on an `lrz-hgx-h100-94x4` node using the 67k swissprot sequences.
     * Result: Out of 8,192 total features, 315 features were non-zero at least once and 7,877 were dead.
...
10. **InterPLM Evaluation Pipeline:** Ran the full evaluation pipeline, including activation comparison, F1 calculation, and metric reporting for both validation and test sets.
    ...
    * **Baseline (67k):** Ran `InterPLM/scripts/submit_eval.sh` for the baseline crosscoder on an `lrz-hgx-h100-94x4` node.
      * Result: Compared 310 features (with 1+ true positive) to 219 concepts. Average best F1 per concept in test set: 0.053. Number of concepts identified: 1. Number of features associated with a concept: 1.
...
11. **Feature Activation Collection:** Ran `InterPLM/scripts/submit_collect.sh` containing `InterPLM/scripts/collect_feature_activations.py` on an `lrz-hgx-h100-94x4` node. This identified the top activating protein sequences and computed statistics for the features in the crosscoder checkpoint.
    ...
    * **Baseline (67k):** 
      * Result: Out of 8,192 total features, 315 features had activations and 7,877 were dead.
...
12. **Dashboard Cache Generation:** Ran `InterPLM/scripts/submit_create_dashboard.sh` containing `InterPLM/scripts/create_dashboard.py` on an `lrz-hgx-h100-94x4` node. This generated the data cache required for interactive visualization in the InterPLM dashboard, integrating the crosscoder features with their biological concepts.
    ...
    * **Baseline (67k):** 
      * Result: Successfully created dashboard cache and integrated concept F1 results for 1 feature.
